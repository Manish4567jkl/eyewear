...
async function loadProduct(prod) {
  ...
  try {
    ...
    model.traverse((object) => {
      ...
      const category = isCorbinTemple ? "handles" : classifyMesh(object, rawModelUrl);
...
// ---------- Clean view ----------
// Hides every overlay (see body.clean-view in lens-detail.html) for an unobstructed shot
// of the product; a double-click anywhere brings it back. Global rather than scoped to
// the stage so it works no matter where the visitor double-clicks, including over what's
// left visible (the hint text itself).
